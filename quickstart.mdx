...
<Steps>
  ...
  <Step title="Set up and start the backend">
    ...
    <Accordion title="Backend startup logs">
      ...
      ```
      INFO  Fetching items from Data Dragon API - Version: 14.1.1, Language: en_US
...
<Accordion title="Backend health check">
  ...
  ```bash
  curl http://localhost:8080/api/game/question?difficulty=MEDIUM
  ```
...
<Accordion title="Frontend connection test">
  ...
  ```
  GET http://localhost:8080/api/game/question?difficulty=MEDIUM 200 OK
  ```
</Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Frontend not connecting to backend">
    ...
    1. Verify the backend is running: `curl http://localhost:8080/api/game/question`
