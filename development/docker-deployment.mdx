...
```bash
docker run -p 5000:8080 crafter-lol-backend:latest
```

<ParamField path="-p 5000:8080" type="string">
  Port mapping: `host:container`
  - Host port 5000 maps to container port 8080
...
services:
  backend:
    ...
    restart: unless-stopped
    healthcheck:
      test: ["CMD", "curl", "-f", "http://localhost:8080/api/game/question"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 40s
